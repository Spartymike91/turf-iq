...
export default function DashboardPage() {
  ...
  // Fast path: weather, today's tasks, equipment issues — no LLM call, so
  // this is what the page waits on before rendering.
  ...
  // Slow path: the AI-generated headline/focus items. Fetched separately so
  // it never blocks the rest of the dashboard from rendering — the banner
  // just shows a neutral placeholder until this resolves.
  const loadHeadline = useCallback(async () => {
    ...
    try {
      ...
      // Best-effort — banner just keeps its neutral placeholder.
    }
  ...
  useEffect(() => {
    async function load() {
      ...
      await loadBriefing();
      setChecking(false);
      // Fire-and-forget — the page is already usable with real data; the AI
      // headline fills in whenever it's ready instead of holding up the rest.
      loadHeadline();
    }
    load();
  ...
  return (
    <>
      ...
      <AlertBanner
        ...
        title={headline ?? (headlineLoading ? "Analyzing today's conditions…" : "Today's briefing")}
      ...
      {focusItems.length > 0 ? (
        ...
        headlineLoading && (
          <div className="bg-white border-[1.5px] border-rule rounded-[10px] p-5">
            <div className="font-serif text-lg text-green-dark mb-3">Today&apos;s Focus</div>
            <div className="text-sm text-mist">Generating…</div>
          </div>
        )
      )}
