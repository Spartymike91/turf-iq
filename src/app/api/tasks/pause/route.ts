...
export async function POST(request: NextRequest) {
  ...
  if (!allowed) {
    return NextResponse.json({ error: "You can only pause tasks assigned to you." }, { status: 403 });
  }

  if (assignment.status !== "in_progress") {
    return NextResponse.json({ error: "Only an in-progress task can be paused." }, { status: 400 });
  }
