...
export default function DiseaseRiskSection() {
  ...
  const [greensGrassTypes, setGreensGrassTypes] = useState<string[]>([]);
  ...
  useEffect(() => {
    async function load() {
      ...
      setGreensGrassTypes(resolveGrassTypes(course?.grass_type_greens, course?.grass_type));
  ...
  const groups = useMemo(() => groupByAppliedAtAndArea(sprays), [sprays]);
  ...
  const showCoolSeasonDiseases = isCoolSeasonGrass(greensGrassTypes);
  const showSpringDeadSpot = isSpringDeadSpotHost(fairwaysGrassTypes);
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <div className="text-[13px] text-mist mt-1">
            {courseName} {greensGrassTypes.length > 0 && `· ${greensGrassTypes.join("/")} greens`} ·{" "}
      ...
      <div id="disease-application-log" className="bg-white border-[1.5px] border-rule rounded-[10px] overflow-hidden shrink-0">
        ...
        {sprays.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            ...
            <tbody>
              {groups.map((group) => {
                const groupCost = group.rows.reduce((sum, s) => sum + Number(s.cost ?? 0), 0);
                return (
                    ...
                    {group.rows.map((s) =>
                      editingId === s.id ? (
                        ...
                        <tr key={s.id} className="border-b border-rule last:border-0">
                          <td className="px-5 py-2.5 font-medium">{s.target || "—"}</td>
