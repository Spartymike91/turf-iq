...
export default function GrowthRegulatorSection() {
  ...
  const growthRegulatorApplications = applications.filter(
    (a) => a.category === "growth_regulator" || (a.category == null && isGrowthRegulatorApplication(a, products))
  );
  const groups = useMemo(() => groupByAppliedAtAndArea(growthRegulatorApplications), [growthRegulatorApplications]);
      ...
      <div id="growth-regulator-application-log" className="bg-white border-[1.5px] border-rule rounded-[10px] overflow-hidden shrink-0">
        ...
        {growthRegulatorApplications.length === 0 ? (
          <div className="p-10 text-center">
            ...
            <div className="text-sm text-mist">No growth regulator applications logged yet. Log one from the button above the tabs.</div>
          </div>
        ) : (
                    ...
                    {group.rows.map((a) =>
                      editingId === a.id ? (
                        <PestApplicationEditRow
                          key={a.id}
                          app={a}
                          resolvedCategory={(a.category as ProductCategory) ?? "growth_regulator"}
                          products={products}
                          colSpan={7}
